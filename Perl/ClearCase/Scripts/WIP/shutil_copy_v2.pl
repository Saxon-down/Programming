...
sub Generate_Path {           # $src, $v, $dst, $elem
  ...
  my ($src, $v, $dst, $elem) = (@_);
  my ($gensrc, $gendst);
  my @sc = split /$OSSEP/, $src;
  my @vc = split /$OSSEP/, $v;
  my @dc = split /$OSSEP/, $dst;
  my @ec = split /$OSSEP/, $elem;
  $gensrc = join($OSSRC, ($src, $elem));
...
sub STC_CCCI {        # $elem
  ...
  if ($result =~ /data identical to predecessor/) {
    $result = qx/cleartool uncheckout -rm \"$e\" 2>&1/);
  } elsif ($result =~ /error/i) {
...
#
  ...
  opendir(my $dir, $path) or {
    &DEBUG($spacer . "  Can't opendir $path: $!");
    &PanicMode("path");
  }
  ...
  foreach my $elem (@children) {
    ...
    if (-d $root . $OSSUB . $elempath) { # Found a directory, so process that too
